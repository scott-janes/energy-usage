...
func (p *Processor) Process(ctx context.Context, message *kafka.Message) {
	...
	result, err := GetMixData(ctx, energyServiceEvent.Context.Date, &logger)
	if err != nil {
		logger.Error().Stack().Err(err).Msg("Error getting C02 data")
		return
	}

  err = StoreData(ctx, result, energyServiceEvent.Context.Date, p.store, &logger)
  if err != nil {
    logger.Error().Stack().Err(err).Msg("Error storing data")
    return
  }
