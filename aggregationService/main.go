...
import (
	...
	"github.com/scott-janes/energy-usage/aggregationService/dao"
...
func main() {
  ...
  log.Info().Msgf("Kafka shizzle %s", appConfig.Kafka.Consumer.Host)
	...
	cache := dao.NewCache(10)

	processor := dao.NewProcessor(store, &appConfig, cache)
			...
			if err != nil {
				if kafkaError, ok := err.(kafka.Error); ok {
					if kafkaError.IsTimeout() {
						// timeout occurred, continue to check context's
						continue
					}
