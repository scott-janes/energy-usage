...
func (p *Processor) Process(ctx context.Context, message *kafka.Message) {
	...
	logger.Info().Msgf(`Test: %s`, p.config.ServiceName)
	...
	result, err := GetConsumptionData(ctx, energyServiceEvent.Context.Date, p.config, &logger)
	if err != nil {
		logger.Error().Stack().Err(err).Msg("Error getting consumption data")
		return
	}

	err = StoreConsumptionData(ctx, result, energyServiceEvent.Context.Date, p.store, &logger)
